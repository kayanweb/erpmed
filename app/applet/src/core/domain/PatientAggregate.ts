...
export class PatientAggregate {
  ...
  // Reconstruct state from an event stream (Event Sourcing)
  ...
  private applyEvent(event: DomainEvent) {
    switch (event.type) {
      ...
      case 'OPD_APPOINTMENT_BOOKED':
        this.state = 'BOOKED_OPD';
        this.currentLocation = 'Home';
        break;

      case 'PATIENT_ARRIVED_OPD':
        this.state = 'WAITING_OPD';
        this.currentLocation = event.payload.clinic;
        break;

      case 'DOCTOR_CONSULTATION_STARTED':
        this.state = 'IN_CONSULTATION';
        this.currentLocation = event.payload.room || this.currentLocation;
        break;

      case 'DOCTOR_CONSULTATION_ENDED':
