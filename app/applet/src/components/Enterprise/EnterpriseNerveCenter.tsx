...
export default function EnterpriseNerveCenter({ language = 'ar' }: Props) {
  ...
  const startOPDJourney = () => {
    EventBus.clearHistory();
    setEvents([]);
    
    const newPatient = new PatientAggregate('PT-1002', 'MRN-8841', isAr ? 'سارة عبدالله' : 'Sarah Abdullah');
    setPatient(newPatient);

    const correlationId = `SAGA-OPD-${Date.now()}`;

    // 1. Appointment booked
    EventBus.publish({
      id: Math.random().toString(36).substr(2, 9),
      type: 'OPD_APPOINTMENT_BOOKED',
      timestamp: new Date().toISOString(),
      source: 'CallCenter_Booking',
      correlationId: correlationId,
      payload: {
        patientId: newPatient.id,
        clinic: 'Cardiology',
    ...
    setTimeout(() => {
      EventBus.publish({
        id: Math.random().toString(36).substr(2, 9),
        type: 'PATIENT_ARRIVED_OPD',
        timestamp: new Date().toISOString(),
        source: 'OPD_Reception_Kiosk',
        correlationId: correlationId,
        payload: {
          patientId: newPatient.id,
          clinic: 'Cardiology',
  ...
  const getServiceColor = (source: string) => {
    ...
    if (source.includes('Reception') || source.includes('CallCenter')) return 'text-indigo-500 bg-indigo-50 border-indigo-200';
    if (source.includes('OPD_Doctor') || source.includes('Nursing')) return 'text-pink-500 bg-pink-50 border-pink-200';
    ...
    <div className="flex flex-col h-full bg-slate-900 text-slate-300 font-mono text-sm" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="bg-slate-950 p-4 border-b border-slate-800 flex items-center justify-between shrink-0">
        ...
        <div className="flex gap-2">
          <button 
            onClick={startOPDJourney}
            className="px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-lg flex items-center gap-2 font-bold transition shadow-lg shadow-pink-500/20"
          >
            <User size={16} className="fill-current" />
            {isAr ? "مسار العيادات (OPD)" : "OPD Journey"}
          </button>
          <button 
          ...
          >
            ...
            {isAr ? "مسار الطوارئ (ER)" : "ER Journey"}
          </button>
        </div>
      </div>
            ...
            {patient ? (
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className={`p-4 rounded-xl border transition-colors ${events.some(e => e.type === 'DOCTOR_CONSULTATION_STARTED') ? 'bg-pink-900/20 border-pink-500/50' : 'bg-slate-800/50 border-slate-700'}`}>
                    <div className="flex items-center gap-2 mb-2 font-bold text-white">
                      <Stethoscope className={events.some(e => e.type === 'DOCTOR_CONSULTATION_STARTED') ? 'text-pink-400' : 'text-slate-500'} size={16} />
                      OPD Clinic Service
                    </div>
                    <div className="text-xs text-slate-400">
                      {events.some(e => e.type === 'DOCTOR_CONSULTATION_STARTED') 
                        ? `Consultation active in ${patient.currentLocation}`
