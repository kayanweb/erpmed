...
const INITIAL_MESSAGES: Record<string, ChatMessage[]> = {
  ...
  "C-3": [
    { 
      ...
      timestamp: 'Yesterday 14:30', status: 'READ', isMe: false, priority: 'CRITICAL', requiresAck: true,
...
export default function ClinicalCommunication({ language, onClose }: Props) {
  ...
  const [showCallModal, setShowCallModal] = useState<{type: 'AUDIO' | 'VIDEO', user: string} | null>(null);
  ...
  const handleAcknowledge = (msgId: string) => {
    ...
    setMessages(prev => {
      ...
      return {
        ...prev,
        [activeChat.id]: currentMsgs.map(m => 
          m.id === msgId ? { ...m, acknowledgedAt: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) } : m
        )
      };
    });
  };
          ...
          <div className={`${activeChat ? 'flex' : 'hidden lg:flex'} flex-1 flex-col min-w-0 bg-white relative`}>
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 flex items-center justify-between bg-white/90 backdrop-blur-md z-10 shadow-[0_2px_10px_rgba(0,0,0,0.02)] sticky top-0">
              ...
              <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
                <button onClick={() => setShowCallModal({type: 'AUDIO', user: activeChat.title})} className="p-2 sm:p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition">
                  <Phone size={18} />
                </button>
                <button onClick={() => setShowCallModal({type: 'VIDEO', user: activeChat.title})} className="p-2 sm:p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition hidden sm:block">
                  <Video size={18} />
                </button>
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="flex-1 flex flex-col relative bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-slate-50/50">
                          ...
                          <div className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[70%]`}>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              >
                                ...
                                {msg.requiresAck && !msg.isMe && (
                                  <div className="mt-3 pt-3 border-t border-rose-100/30">
                                    {msg.acknowledgedAt ? (
                                      <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-bold text-rose-700/80">
                                        <CheckCircle2 size={14} />
                                        {isAr ? `تم الإقرار بواسطة أنت - ${msg.acknowledgedAt}` : `Acknowledged by You - ${msg.acknowledgedAt}`}
                                      </div>
                                    ) : (
                                      <button
                                        onClick={() => handleAcknowledge(msg.id)}
                                        className="w-full flex items-center justify-center gap-2 py-2 px-3 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold transition shadow-sm"
                                      >
                                        <CheckCircle2 size={16} />
                                        {isAr ? "إقرار بالنتيجة الحرجة" : "Acknowledge Critical Result"}
                                      </button>
                                    )}
                                  </div>
                                )}
                ...
                {/* Input Area */}
                <div className="p-3 sm:p-4 bg-white border-t border-slate-200 z-20 flex flex-col gap-3">
                  
                  {/* Quick Replies */}
                  <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar pb-1">
                    {[
                      { ar: "قادم للتو", en: "On my way" },
                      { ar: "استلمت", en: "Acknowledged" },
                      { ar: "تم تنفيذ الطلب", en: "Order placed" },
                      { ar: "أحتاج مساعدة", en: "Need assistance" }
                    ].map((qr, i) => (
                      <button 
                        key={i} 
                        onClick={() => setMessageInput(isAr ? qr.ar : qr.en)}
                        className="px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold rounded-full whitespace-nowrap transition-colors border border-indigo-100 shadow-sm"
                      >
                        {isAr ? qr.ar : qr.en}
                      </button>
                    ))}
        ...
        {showCallModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-sm bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col items-center shadow-[0_20px_60px_rgba(0,0,0,0.5)] overflow-hidden"
            >
              ...
              <div className="w-24 h-24 rounded-full bg-slate-800 border-4 border-slate-700 flex items-center justify-center mb-6 relative z-10 shadow-xl">
                <User size={40} className="text-slate-400" />
              </div>
              <h3 className="text-2xl font-bold text-white mb-2 relative z-10 text-center">{showCallModal.user}</h3>
              <p className="text-indigo-300 text-sm font-medium tracking-widest uppercase mb-12 relative z-10">
                {showCallModal.type === 'VIDEO' ? (isAr ? 'مكالمة فيديو صادرة...' : 'Calling Video...') : (isAr ? 'مكالمة صوتية صادرة...' : 'Calling Audio...')}
              </p>

              <div className="flex items-center gap-6 relative z-10 mt-auto">
                <button onClick={() => setShowCallModal(null)} className="w-14 h-14 rounded-full bg-rose-600 hover:bg-rose-700 text-white flex items-center justify-center shadow-lg transition-transform hover:scale-105">
