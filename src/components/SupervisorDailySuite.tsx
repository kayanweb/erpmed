...
export default function SupervisorDailySuite({ language }: Props) {
  ...
  const { getSetting, saveSetting, currentUser } = useHIS();
  ...
  useEffect(() => {
    getSetting("suite_supervisor_assignment").then(val => val && setAssignment(val));
    getSetting("suite_supervisor_daily").then(val => val && setDailyReport(val));
    getSetting("suite_supervisor_census").then(val => val && setCensus(val));
    getSetting("suite_supervisor_checklists").then(val => val && setChecklists(val));
    getSetting("suite_supervisor_indicators").then(val => val && setClinicalIndicators(val));
  }, [getSetting]);

  const handleSaveAll = () => {
    saveSetting("suite_supervisor_assignment", assignment);
    saveSetting("suite_supervisor_daily", dailyReport);
    saveSetting("suite_supervisor_census", census);
    saveSetting("suite_supervisor_checklists", checklists);
    saveSetting("suite_supervisor_indicators", clinicalIndicators);
  ...
  const handleResetToDefaults = () => {
    if (window.confirm(isAr ? "هل أنت متأكد من استعادة بيانات المستشفى النموذجية الأصلية؟" : "Reset current worksheet to default template?")) {
      ...
      toast.success(isAr ? "تم استرداد قوالب النماذج الأصلية بنجاح!" : "Original Hospital templates loaded!");
    }
  };
    ...
    <div className="bg-slate-50 min-h-screen p-4 md:p-6 text-slate-800 font-sans" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {/* Enterprise Electronic Signature & Approval Panel */}
          <ClinicalSignaturePanel
            language={language}
            currentUser={currentUser}
            isSigned={Boolean((dailyReport as any)?.isSigned)}
            signatureData={(dailyReport as any)?.signature || null}
            ...
            onSign={(signatureData) => {
              ...
              saveSetting("suite_supervisor_daily", updatedReport);
              setIsEditMode(false);
            }}
            onUnlock={() => {
              const updatedReport = {
            ...
            titleAr="توقيع واعتماد نموذج السوبرفايزر الموحد"
            titleEn="Unified Supervisor Daily Suite Electronic Approval"
            entityType="SUPERVISOR_DAILY_SUITE"
            entityId={(dailyReport as any)?.date || "SUITE-LOG"}
            requiredRoles={['supervisor', 'cno', 'nursing_director', 'admin', 'quality', 'head_nurse']}
          />
