...
export default function ProfileView({ user, language, hospitalSettings, systemUsers = [], currentUser, onClose }: ProfileViewProps) {
  ...
  // Settings Toggles (saved in database for persistence)
  const [denseLayout, setDenseLayout] = useState(false);
  const [playNoises, setPlayNoises] = useState(true);
  const [printPaperSize, setPrintPaperSize] = useState("A4");

  useEffect(() => {
    getSetting("pref_dense_layout").then(val => val !== undefined && setDenseLayout(val === true));
    getSetting("pref_play_noises").then(val => val !== undefined && setPlayNoises(val !== false));
    getSetting("pref_print_paper_size").then(val => val && setPrintPaperSize(val));
  }, [getSetting]);
  ...
  // Load activity metrics and sub-lists
  useEffect(() => {
    getSetting("hospital_medical_records").then(records => {
      const userRecords = (records || []).filter((rc: any) => rc.authorId === user.id || rc.authorNameEn === user.nameEn);
      setMetrics({
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      saveSetting("pref_dense_layout", denseLayout);
      saveSetting("pref_play_noises", playNoises);
      saveSetting("pref_print_paper_size", printPaperSize);
