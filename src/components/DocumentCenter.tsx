...
import { saveCloudDocument, syncCloudDocuments, deleteCloudDocument, saveHospitalSettings, syncSetting, saveCustomTemplate } from "../lib/firestoreService";
...
export default function DocumentCenter({ language, currentUser, systemUsers }: DocumentCenterProps) {
  ...
  const handleConvertToGrid = (doc: CloudDocument) => {
    try {
        ...
        saveCustomTemplate(newTemplate).then(async () => {
          // Tell App.tsx to push it locally if we're not waiting for listener
          let currentTemplates: FormTemplate[] = await getSetting("baheya_custom_templates") || [];
          currentTemplates.push(newTemplate);
          await saveSetting("baheya_custom_templates", currentTemplates);
