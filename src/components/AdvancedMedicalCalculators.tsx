...
export default function AdvancedMedicalCalculators({
  ...
  const [hospitalName, setHospitalName] = useState<string>("مستشفى الرعاية السريرية");

  useEffect(() => {
    getSetting("baheya_hospital_settings").then(saved => {
      if (saved && saved.nameAr) setHospitalName(saved.nameAr);
    });
  }, [getSetting]);
  ...
  useEffect(() => {
    getSetting("hospital_isbar_handovers").then(saved => {
      if (saved) {
        setHandoverList(saved);
      } else {
        ...
        saveSetting("hospital_isbar_handovers", seed);
      }
    });
  }, [getSetting]);
  ...
  const handleAddHandover = async (e: React.FormEvent) => {
    ...
    await saveSetting("hospital_isbar_handovers", updated);
  ...
  const handleDeleteHandover = async (id: string) => {
    if (confirm(isAr ? "هل أنت متأكد من حذف تقرير التسليم هذا؟" : "Are you sure you want to delete this handover?")) {
      ...
      await saveSetting("hospital_isbar_handovers", filtered);
...
`;
  ...
  useEffect(() => {
    getSetting("hospital_crash_cart_audit").then(saved => {
      if (saved) setCrashCartAudit(saved);
    });
  }, [getSetting]);

  const handleSaveCrashCartAudit = async () => {
    ...
    await saveSetting("hospital_crash_cart_audit", nextObj);
            ...
            <div className="mt-6 p-4 bg-white border rounded-2xl flex items-center justify-between">
              ...
              <div className="text-right text-[11px] leading-relaxed">
                <span className="font-black text-slate-800 block">
                  {isAr ? `معايرة دورية تابعة للائحة JCI بـ ${hospitalName}` : "Relational clinical compliance lock standard"}
                </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={idx} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 hover:border-pink-300 hover:shadow-md transition-all group flex flex-col justify-between">
                ...
                <button
                   onClick={() => toast.info(isAr ? `تفعيل أداة: ${tool.ar}` : `Executing clinical tool: ${tool.en}`)}
