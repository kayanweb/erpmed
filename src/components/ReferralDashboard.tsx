...
import { ArrowRightLeft, Search, Filter, Plus, Clock, Activity, FileText, CheckCircle, AlertTriangle, ArrowRight, Server, Shield } from 'lucide-react';
...
export default function ReferralDashboard({ language, onClose }: Props) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await firestoreService.getAll<RecordType>('ReferralDashboard');
        setRecords(data);
      } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <ArrowRightLeft className="w-8 h-8 text-indigo-600 bg-indigo-100 p-1.5 rounded-xl" />
            {isAr ? "الإحالات" : "Referrals"}
          </h1>
          <p className="text-sm font-medium text-slate-500 mt-1">
            {isAr ? "الإحالات الصادرة والواردة للمرضى" : "Inbound and outbound patient referrals"}
          </p>
        </div>
