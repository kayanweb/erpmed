...
import { Users, Search, Filter, Plus, Clock, Activity, FileText, CheckCircle, AlertTriangle, ArrowRight, Server, Shield } from 'lucide-react';
...
export default function QueueManagementDashboard({ language, onClose }: Props) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await firestoreService.getAll<RecordType>('QueueManagementDashboard');
        setRecords(data);
      } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            ...
            {isAr ? "إدارة الطوابير" : "Queue Management"}
          </h1>
          <p className="text-sm font-medium text-slate-500 mt-1">
            {isAr ? "تدفق المرضى ومراقبة مناطق الانتظار" : "Patient flow and waiting area monitoring"}
          </p>
        </div>
