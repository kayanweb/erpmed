...
export default function RosterPlanningPanel({
  ...
  const { getSetting, saveSetting, syncSetting } = useHIS();
  ...
  const [settings, setSettings] = useState<RosterGeneralSettings>(defaultGeneralSettings);

  useEffect(() => {
    getSetting("hospital_roster_general_settings").then(val => {
  ...
  useEffect(() => {
    getSetting("hospital_roster_overrides").then(val => {
      if (val) setOverridesList(val);
    });
  }, [getSetting]);
  ...
  useEffect(() => {
    getSetting("hospital_daily_attendance").then(val => {
      if (val) setAttendanceTable(val);
    });
  }, [getSetting]);
  ...
  useEffect(() => {
    getSetting("hospital_favorite_days_templates").then(val => {
      if (val) setSavedTemplates(val);
    });
  }, [getSetting]);
