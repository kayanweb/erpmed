...
import { CheckSquare, Search, Filter, Plus, Clock, Activity, FileText, CheckCircle, AlertTriangle, ArrowRight, Server, Shield } from 'lucide-react';
...
export default function SmartChecklistEngine({ language, onClose }: Props) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await firestoreService.getAll<RecordType>('SmartChecklistEngine');
        setRecords(data);
      } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <CheckSquare className="w-8 h-8 text-indigo-600 bg-indigo-100 p-1.5 rounded-xl" />
            {isAr ? "القوائم الذكية" : "Smart Checklists"}
          </h1>
          <p className="text-sm font-medium text-slate-500 mt-1">
            {isAr ? "قوائم التحقق الديناميكية للإجراءات" : "Dynamic checklists for procedures"}
          </p>
        </div>
