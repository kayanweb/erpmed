...
export default function QualityAnalyticsHub({
  ...
  // helper sync calls to cache - REMOVED LOCALSTORAGE AS PER USER REQUEST FOR PERSISTENT DB ONLY
  useEffect(() => {
    // Sync to persistent DB settings is already handled via saveSetting inside handle functions
  }, [ovrs, staffEvals, unitAudits, gaharChecked, acknowledgedPolicies]);
            ...
            <button
              onClick={() => {
                if (confirm(isAr ? "هل أنت متأكد من مسح جميع التقارير الجارية؟" : "Are you sure you want to clear all records?")) {
                  setRecords([]);
                  saveSetting("hospital_medical_records", []);
