...
// Helper function to safely invoke doc.text without throwing on null, undefined, non-numeric positions, or unhandled unicode
function safeAddText(doc: jsPDF, text: any, x: number, y: number, options?: any) {
  const str = String(text ?? "");
  // Strip Arabic/non-Latin characters for standard jsPDF helvetica font to prevent encoding errors
  const cleanStr = str.replace(/[\u0600-\u06FF]/g, "").trim() || (str.length > 0 ? "Report" : "");
  const posX = isFinite(Number(x)) ? Number(x) : 10;
  const posY = isFinite(Number(y)) ? Number(y) : 10;
  try {
    if (options) {
      doc.text(cleanStr, posX, posY, options);
    } else {
      doc.text(cleanStr, posX, posY);
    }
  } catch (e) {
    console.error("jsPDF text error caught:", e);
  }
}
...
export function generatePDF(
  ...
  const isLandscape = dayFocus === "all" && record?.gridData && Array.isArray(record.gridData) && record.gridData.some(row => row?.days && Object.keys(row.days).length > 0);
  ...
  const nameEn = hospitalSettings?.nameEn || "Hospital";
  const taglineEn = hospitalSettings?.taglineEn || "";
  const emergencyPhone = hospitalSettings?.emergencyPhone || "N/A";
  const address = hospitalSettings?.address || "Address N/A";

  // Align English title left, Arabic text right if portrait
  if (isLandscape) {
    safeAddText(doc, nameEn, 12, 15);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    safeAddText(doc, taglineEn, 12, 20);
    
    // Arabic fallback bilingual titles drawn on the right safely
    doc.setFontSize(14);
    safeAddText(doc, nameEn, pageWidth - 12 - 55, 15);
    doc.setFontSize(8);
    safeAddText(doc, "Clinical Quality Operations Protocol", pageWidth - 12 - 55, 20);
  } else {
    safeAddText(doc, nameEn, 12, 15);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    ...
    // Right side
    safeAddText(doc, `Hotline: ${emergencyPhone}`, pageWidth - 50, 15);
    safeAddText(doc, address, pageWidth - 80, 20);
  }
  ...
  const sheetTitle = `${template?.code || "FORM"} - ${template?.titleEn || "Medical Report"}`;
  safeAddText(doc, sheetTitle, 12, metaY);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(secondaryColor[0], secondaryColor[1], secondaryColor[2]);
  const verText = `Version: ${template?.version || "01"} | Release Date: ${template?.issueDate || "03.2025"}`;
  safeAddText(doc, verText, 12, metaY + 4.5);
  ...
  // Row 1
  doc.setFont("helvetica", "bold");
  safeAddText(doc, "Department/Unit:", 13, lineY);
  doc.setFont("helvetica", "normal");
  safeAddText(doc, record?.department || template?.departmentDefault || "N/A", 40, lineY);

  doc.setFont("helvetica", "bold");
  safeAddText(doc, "Audit Date:", 13 + colWidth * 1.5, lineY);
  doc.setFont("helvetica", "normal");
  safeAddText(doc, record?.date || new Date().toISOString().slice(0, 10), 13 + colWidth * 1.5 + 20, lineY);

  doc.setFont("helvetica", "bold");
  safeAddText(doc, "Audit Time / Shift:", 13 + colWidth * 2.8, lineY);
  doc.setFont("helvetica", "normal");
  safeAddText(doc, `${record?.time || "N/A"} ${selectedShift ? `(${selectedShift})` : ""}`, 13 + colWidth * 2.8 + 32, lineY);

  // Row 2 for Portrait
  if (!isLandscape) {
    const line2Y = cardY + 11;
    doc.setFont("helvetica", "bold");
    safeAddText(doc, "Auditor Name:", 13, line2Y);
    doc.setFont("helvetica", "normal");
    safeAddText(doc, record?.staffName || "System Staff", 35, line2Y);

    doc.setFont("helvetica", "bold");
    safeAddText(doc, "ID Code / PIN Status:", 13 + colWidth * 1.5, line2Y);
    doc.setFont("helvetica", "normal");
    safeAddText(doc, `${record?.staffId || "N/A"} - Secured Login`, 13 + colWidth * 1.5 + 32, line2Y);

    if (record?.patientName || record?.patientMRN) {
      doc.setFont("helvetica", "bold");
      safeAddText(doc, "Patient Name/MRN:", 13 + colWidth * 2.8, line2Y);
      doc.setFont("helvetica", "normal");
      safeAddText(doc, `${record?.patientMRN || "N/A"} - ${record?.patientName || ""}`, 13 + colWidth * 2.8 + 32, line2Y);
    }
  } else {
    // If landscape, we can write auditor info in Row 1 also
    doc.setFont("helvetica", "bold");
    safeAddText(doc, "Auditor:", 13, cardY + 10);
    doc.setFont("helvetica", "normal");
    safeAddText(doc, `${record?.staffName || "System Staff"} (Code: ${record?.staffId || "N/A"})`, 30, cardY + 10);
    
    doc.setFont("helvetica", "bold");
    safeAddText(doc, "Date Resolution Scope:", 13 + colWidth * 1.8, cardY + 10);
    doc.setFont("helvetica", "normal");
    safeAddText(doc, `${dayFocus === "all" ? `Whole Month Grid (31 Days Matrix for ${record?.date ? record.date.slice(0, 7) : ""})` : `Focused Day ${dayFocus}`}`, 13 + colWidth * 1.8 + 35, cardY + 10);
  }
  ...
  if (dayFocus !== "all") {
    ...
    tableRows = gridData.map((row, idx) => [
      row?.sn || (idx + 1).toString(),
      row?.code || "-",
      `${row?.itemEn || ""} \n(${row?.itemAr || ""})`,
      row?.unit || "PCS",
      row?.qty || "1",
      row?.days ? (row.days[dayFocus.toString()] || "") : ""
    ]);
  } else {
    ...
    tableRows = gridData.map((row) => {
      const dayValues = dayCols.map(d => (row?.days ? (row.days[d] || "") : ""));
      return [
        row?.code || "-",
        `${row?.itemEn || ""}`,
        row?.unit || "PCS",
  ...
  let signatureY = (doc as any).lastAutoTable?.finalY ? (doc as any).lastAutoTable.finalY + 12 : 120;
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(darkSlateColor[0], darkSlateColor[1], darkSlateColor[2]);
  safeAddText(doc, "OFFICIAL ELECTRONIC SIGN-OFF & ACCREDITATION", 14, signatureY + 5);
  ...
  safeAddText(doc, signTextLeft, 14, signatureY + 10);
  ...
  safeAddText(doc, signTextRight, pageWidth / 2 + 10, signatureY + 10);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
  safeAddText(doc, `${(nameEn).toUpperCase()} CLINICAL AUDIT`, pageWidth - 62, signatureY + 7);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6.5);
  safeAddText(doc, "SIGNED ELECTRONICALLY BY THE SYSTEM", pageWidth - 62, signatureY + 11);
  safeAddText(doc, "VERIFIED ELECTRONIC STAMP", pageWidth - 62, signatureY + 15);

  // 5. FOOTER DETAILS
  const defaultFooterAr = `Quality & Clinical Standards Unit - System Certified Report`;
  const defaultFooterEn = `${nameEn} Quality & Clinical Standards Unit - Certified System Report`;
  
  const customFooter = language === "ar" 
    ? (hospitalSettings?.footerAr || defaultFooterAr)
    : (hospitalSettings?.footerEn || defaultFooterEn);
  const footerText = `${customFooter} | ${address} | Tel: ${emergencyPhone}`;
  doc.setFontSize(7);
  doc.setTextColor(148, 163, 184); // slate-400
  safeAddText(doc, footerText, pageWidth / 2, pageHeight - 8, { align: "center" });

  // Save PDF
  doc.save(`${template?.code || "REPORT"}_report_${record?.date || "daily"}.pdf`);
}
